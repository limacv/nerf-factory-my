...
import src.model.specnerf.helper as helper
import src.model.specnerf.spec_utils as spec_utils
import utils.store_image as store_image
from src.model.interface import LitModel


@gin.configurable()
class SpecNeRFMLP(nn.Module):
    def __init__(
        self,
        deg_view,
        min_deg_point: int = 0,
        ...
        super(SpecNeRFMLP, self).__init__()
        init_translation = torch.randn(deg_view, 3)
        init_translation = init_translation / (torch.norm(init_translation, dim=-1, keepdim=True) + 0.2 * torch.randn_like(init_translation[:, :1]))
        init_translation = init_translation * 50
        init_scale = torch.ones_like(init_translation) * 20
        self.dir_enc_fn = spec_utils.LearnableSphericalGaussianEncoding(
            deg_view,
            init_translation = init_translation,
            init_scale = init_scale
            )
        self.dir_enc_fn = torch.compile(self.dir_enc_fn)
    ...
    def forward(self, samples, viewdirs):
        ...
        with torch.set_grad_enabled(True):
            ...
            normals = -spec_utils.l2_normalize(raw_density_grad)
            means.detach()
...
class SpecNeRF(nn.Module):
    def __init__(
        ...
        super(SpecNeRF, self).__init__()

        self.mlp = SpecNeRFMLP(self.deg_view)
...
class LitSpecNeRF(LitModel):
    def __init__(
        ...
        super(LitSpecNeRF, self).__init__()
        self.model = SpecNeRF()
    ...
    def compute_normal_mae(self, rendered_results, normals):
        ...
        normalized_normals_gt = spec_utils.l2_normalize(normals_gt[..., None, :])
        normalized_normals = spec_utils.l2_normalize(rendered_results[1]["normals"])
        normal_mae = spec_utils.compute_weighted_mae(
